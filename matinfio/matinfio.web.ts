...
namespace $ {
	...
	export class $mpds_cifplayer_matinfio extends $mol_object2 {
		...
		static detect_format( str ) {
			...
			for( var i = 6; i < 9; i++ ) {
				if( !lines[ i ] ) break
				if( [ 'direct', 'cartes' ].indexOf( trim( lines[ i ] ).toLowerCase().substr( 0, 6 ) ) !== -1 ) return 'POSCAR'
			}
		...
		static jsobj2cif( crystal ) {
			...
			if( Object.keys( crystal.cell ).length == 6 ) {
				...
				cell_mat = this.cell2vec( crystal.cell )
			} else {
				cell_abc = this.vec2cell( crystal.cell )
		...
		static poscar2jsobj( str ) {
			...
			for( ; i < len; i++ ) {
				if( i == 0 ) {
					...
					marker = trim( lines[ i ] ).toLowerCase().substr( 0, 6 )
		...
		static to_player( str ) {
			...
			switch( format ) {
				...
				case 'OPTIMADE': structure = this.optimade2jsobj( str ); break
		...
		static to_cif( str ) {
			...
			if( !structure ) return false
			return this.jsobj2cif( structure )
		}
