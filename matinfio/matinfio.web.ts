...
namespace $ {
	...
	export class $mpds_cifplayer_matinfio extends $mol_object2 {
		...
		static cell2vec( a, b, c, alpha, beta, gamma ) {
		...
		static jsobj2player( crystal ) {
			var cell,
				descr = false
			if( Object.keys( crystal.cell ).length == 6 ) { // for CIF
				cell = this.cell2vec( crystal.cell.a, crystal.cell.b, crystal.cell.c, crystal.cell.alpha, crystal.cell.beta, crystal.cell.gamma )
			...
			for( i; i < len; i++ ) {
				...
				if( hashes.hasOwnProperty( k ) ) {
					...
					for( oprop in player_output.atoms[ hashes[ k ] ].overlays ) {
						...
						player_output.atoms[ hashes[ k ] ].overlays[ oprop ] += update
					}
				} else {
					...
					var cpos = Mimpl.multiply( pos, cell )
					player_output.atoms.push( { "x": cpos[ 0 ], "y": cpos[ 1 ], "z": cpos[ 2 ], "c": color, "r": radius, "overlays": optionpanel } )
					hashes[ k ] = player_output.atoms.length - 1
				}
			}
			for( var opt in optionpanel ) {
				if( opt !== "S" && opt !== "N" ) player_output.overlayed[ opt ] = custom_atom_loop_props[ opt ]
			}
			//console.log(player_output);
			return player_output
		}

		static jsobj2flatten( crystal ) {
		...
		static cif2jsobj( str ) {
			...
			for( i; i < len; i++ ) {
				...
				if( startswith( fingerprt, 'data_' ) ) {
					...
					loop_active = false
					cur_structure.sg_name = trim( lines[ i ].substr( 31 ).replace( /"/g, '' ).replace( /'/g, '' ) )
					continue
					...
					loop_active = false
					line_data = cur_line.split( " " )
					cur_structure.ng_name = trim( line_data[ line_data.length - 1 ] )
					continue
		...
		static poscar2jsobj( str ) {
			...
			for( i; i < len; i++ ) {
				if( i == 0 ) {
					...
					if( trim( lines[ i ] ).toLowerCase().substr( 0, 6 ) == 'direct' ) continue loop_poscar_parse
					...
					if( i < 9 ) {
						...
						if( [ 'direct', 'select' ].indexOf( trim( lines[ i ] ).toLowerCase().substr( 0, 6 ) ) !== -1 ) continue loop_poscar_parse
					}
		...
		static to_player( str ) {
			...
			return this.jsobj2player( structure )
		}
		static to_flatten( str ) {
			var structure,
				format = this.detect_format( str )
			switch( format ) {
				case 'CIF': structure = this.cif2jsobj( str ); break
			...
			if( !structure ) return false
			return this.jsobj2flatten( structure )
		}
